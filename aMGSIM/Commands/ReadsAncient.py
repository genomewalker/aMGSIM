...
def prepare_data_fragments(x, frag_type, tmp_dir, ):
    fa_suffix = ".fasta"
    ...
    fragSim_fname = "{}---{}___fragSim-{}".format(
    ...
    # Save fragmets
    fragment_data["fragSim_frag_ofile"] = Path(
        tmp_dir, fragSim_fname).with_suffix(fragSim_frag_suffix)

    fragments.to_csv(
        path_or_buf=fragment_data["fragSim_frag_ofile"], sep="\t", header=False, index=False)

    fragment_data["fragSim_ofile"] = Path(
        tmp_dir, fragSim_fname).with_suffix(fa_suffix_gz)

    deamSim_fname = "{}---{}___deamSim-{}".format(
        x["comm"], x["taxon"], frag_type)

    fragment_data["deamSim_ofile"] = Path(
        tmp_dir, deamSim_fname).with_suffix(fa_suffix_gz)

    adptSim_fname = "{}---{}___adptSim-{}".format(
        x["comm"], x["taxon"], frag_type)

    fragment_data["adptSim_ofile"] = Path(
        tmp_dir, adptSim_fname).with_suffix(fa_suffix)

    art_fname = "{}---{}___art-{}.".format(
        x["comm"], x["taxon"], frag_type)

    fragment_data["art_ofile"] = Path(
        tmp_dir, art_fname)
...
def collect_file_names(x, art_ofile, fragSim_ofile, deamSim_ofile, adptSim_ofile, frag_type, library):
    if library == "pe":
...
def generate_fragments(x, fragSim_exe, fragSim_params, deamSim_exe, deamSim_params, adptSim_exe, adptSim_params, art_exe, art_params, tmp_dir, exp_data, debug, genome_table):

    genome_data = genome_table[(genome_table["Taxon"] == x["taxon"])]
    fasta = genome_data['Fasta'].item()
    # Create index
    ...
    if x['onlyAncient'] is False:
        if x['fragments_ancient'] is not None:
            ...
            files_ancient = collect_file_names(fragSim_ofile=frag_data["fragSim_ofile"],
        ...
        if x['fragments_modern'] is not None:
            ...
            run_art(exe=art_exe,
                    ...
                    library=library,
                    debug=debug)
            files_modern = collect_file_names(fragSim_ofile=frag_data["fragSim_ofile"],
                                              deamSim_ofile=frag_data["deamSim_ofile"],
                                              adptSim_ofile=frag_data["adptSim_ofile"],
                                              library=library,
                                              art_ofile=frag_data['art_ofile'],
...
def get_ancient_genomes_data(self):
    self = {k: self.get(k, None) for k in (
        "comm", "taxon", "onlyAncient", "library", "seqSys", "coverage")}
    return(self)
...
def rename_reads(records, pattern, comm, taxon, frag_type, fastx):
    for i, record in records:
        ...
        if fastx == "fastq":
            name = '{}__{}--{}:{}:{}:{}:{}:{}:{}/{}'.format(comm, taxon, i, frag_type, m1.group(2),
                                                            m1.group(3), m1.group(4), m1.group(5), m1.group(6), m1.group(7))
        elif fastx == "fasta":
            name = '{}__{}--{}:{}:{}:{}:{}:{}:{}'.format(comm, taxon, i, frag_type, m1.group(2),
                                                         m1.group(3), m1.group(4), m1.group(5), m1.group(6))
        record.id = name
    ...
    with open(output_file, 'w') as outFH:
        for in_file in read_files:
            ...
            with _open(in_file) as f:
                ...
                SeqIO.write(rename_reads(records, pattern=p1, comm=comm,
                                         taxon=taxon, frag_type=frag_type, fastx=fastx), outFH, fastx)
...
def _combine_fasta_types(file_type, data_modern, data_ancient, output_dir, comm, suffix):
    if suffix == "deamSim":
...
def combine_fastx_files(x, output_dir, ancient_files, modern_files, exp_data):
    ...
    if any(x in file_type for x in ["r1", "r2"]):
        files = _combine_fastq_types(file_type=file_type,
        ...
        files = _combine_fasta_types(file_type=file_type,
                                     suffix=files[file_type],
                                     data_modern=data_modern,
                                     data_ancient=data_ancient,
                                     output_dir=output_dir,
...
def main(args):
    ...
    func = partial(generate_fragments,
    ...
    if debug is True:
        ...
        files = list(tqdm.tqdm(p.imap_unordered(
            func, ancient_genomes_data), total=len(ancient_genomes_data)))
    ...
    comms = list(set([s['comm'] for s in ancient_files] +
                     [s['comm'] for s in modern_files]))

    output_dir = config_params['output_dir']
    if not os.path.isdir(output_dir):
        os.makedirs(output_dir, exist_ok=True)

    func = partial(combine_fastx_files,
                   output_dir=output_dir,
