...
def select_genomes(self, parms):
    ...
    while (k_filt <= k):
        ...
        self.loc[(self['Rank']/self['Rank'].max()) <= rank_cutoff, 'onlyAncient'] = bool(False)
   ...
   # random_genomes = withAncient[withAncient['Taxon'].isin(random_genomes)].copy()
    ...
    self.loc[self['onlyAncient'].isin([True]), 'coverage_ancient'] = self['max_ancient_cov_allowed']
    return(self)
...
def _get_read_abundances(self, n_reads):
    ...
    for p in self:
        ...
        if p.fragments_modern is None:
            ...
            _df  = pd.DataFrame([data_dict], columns=data_dict.keys())
            df = df.append(_df, ignore_index=True, sort=False)
    df['seq_depth_rounded'] = df.groupby('comm')['seq_depth'].transform(round_to_nreads, n_reads=n_reads)
    return(df)

def refine_abundances(data, n_reads):
    df = _get_read_abundances(data, n_reads = n_reads)
    df_taxo = df.groupby(['comm', 'taxon'])['seq_depth_rounded'].sum().reset_index(name='seq_depth')
    
    for p in data:
        if p.fragments_ancient is not None:
            df_sub = df.loc[(df["comm"] == str(p.comm)) & (df["taxon"] == p.taxon) & (df["frag_type"] == "ancient")]
            p.fragments_ancient['seq_depth'] = int(df_sub["seq_depth_rounded"].values)
        if p.fragments_modern is not None:
            df_sub = df.loc[(df["comm"] == str(p.comm)) & (df["taxon"] == p.taxon) & (df["frag_type"] == "modern")]
            p.fragments_modern['seq_depth'] = int(df_sub["seq_depth_rounded"].values)
            
        seq_depth = df_taxo.loc[(df_taxo["comm"] == str(p.comm)) & (df_taxo["taxon"] == p.taxon)]['seq_depth'].values
...
def round_to_nreads(number_set, n_reads, digit_after_decimal=0):
    ...
    """
    unround_numbers = [x / float(sum(number_set)) * n_reads * 10 ** digit_after_decimal for x in number_set]
    ...
    remainder = n_reads * 10 ** digit_after_decimal - sum([int(x) for x in unround_numbers])
    index = 0
    while remainder > 0:
...
def main(args):
    ...
    if config['abund_table'] and config['genome_comp']:
        raise IOError("You cannot provide an abundance table and genome composition table at the same time.")
    ...
    # 
    suffix_tsv = ".tsv"
    out_file_tsv = Path(dir_name, "{}_read-abundances".format(file_name)).with_suffix(suffix_tsv)
    df_tsv.to_csv(
        path_or_buf=out_file_tsv, sep="\t", header=True, index=False)
